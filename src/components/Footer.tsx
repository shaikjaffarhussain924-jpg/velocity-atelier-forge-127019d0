    ...
    <footer className="border-t border-border bg-background">
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
          <div>
            <p className="font-heading text-lg font-bold tracking-[0.2em] uppercase text-foreground mb-4">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {["Book Appointment", "Workshop", "Testimonials", "Contact"].map((s) => (
                <li key={s}>
                  <a href={`#${s.toLowerCase().replace(" ", "")}`} className="text-sm text-muted-foreground hover:text-foreground transition-colors">{s}</a>
                </li>
              ))}
            </ul>
          </div>
